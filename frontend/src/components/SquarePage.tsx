  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 overflow-y-auto">
      ...
      {loading ? <div className="flex-1 flex items-center justify-center"><div className="animate-spin rounded-full h-8 w-8 border-2 border-ink-border border-t-coral"/></div> :
       ...
       <><div className="p-6 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">{stories.map(s=><button key={s.id} onClick={()=>openDetail(s.id)} className="text-left border border-ink-border rounded-xl overflow-hidden hover:border-coral/30 hover:bg-ink-light/50 transition-all group"><div className="aspect-[3/4] bg-ink-light overflow-hidden">{s.cover_url?<img src={refImageUrl(s.cover_url)} alt={s.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300"/>:<div className="w-full h-full flex items-center justify-center text-ink-muted"><BookOpenText size={28}/></div>}</div><div className="p-2.5"><h3 className="font-medium text-sm text-cream truncate">{s.title}</h3><p className="text-xs text-cream-dim mt-1 line-clamp-2">{s.description}</p></div></button>)}</div>
